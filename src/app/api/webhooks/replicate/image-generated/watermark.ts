...
export async function addWatermark(imageUrl: string) {
  ...
  const watermarkPath = path.join(
    process.cwd(),
    "src/app/api/webhooks/replicate/image-generated",
    "watermark.png",
  );
  ...
  image.composite([{ input: transparentWatermark, gravity: "southeast" }]);

  // Convert Sharp image to Buffer
  const watermarkedImageBuffer = await image.toBuffer();

  // Create a Blob from the Buffer
  const watermarkedBlob = new Blob([watermarkedImageBuffer], {
    type: "image/webp",
  }); // Adjust the MIME type if necessary

  // Create a File from the Blob
  const file = new File([watermarkedBlob], "watermarked-image.webp", {
    type: "image/webp",
  });
  return file;
}
